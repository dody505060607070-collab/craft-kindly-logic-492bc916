-- Drop both versions of the function to resolve the ambiguity
DROP FUNCTION IF EXISTS public.generate_access_codes(integer, uuid, text, text, integer);
DROP FUNCTION IF EXISTS public.generate_access_codes(uuid, integer, text, text, integer);

-- Re-create the function with a single stable signature
CREATE OR REPLACE FUNCTION public.generate_access_codes(
  _course_id uuid, 
  _count integer, 
  _plan text DEFAULT 'month'::text, 
  _note text DEFAULT NULL::text, 
  _duration_days integer DEFAULT NULL::integer
)
