...
## 5) النسخة الكاملة للباك اند داخل الريبو
- بيانات كل الجداول: `supabase/seed/data.sql`
- كل الملفات المرفوعة (فيديو/صور/PDF): `supabase/storage-backup/` + `manifest.json` (الملفات الكبيرة كـ `*.asset.json` فيها لينك التحميل)
- خطوات الاسترجاع كاملة: `supabase/restore.md` وسكربت `supabase/restore_storage.py`
